{{-- Mobile POS Bottom Nav — §6 Bottom Nav Bar --}}
@php
    $active = $active ?? 'home';
    $navItems = [
        ...
        ['key' => 'sell', 'label' => 'Sell', 'route' => 'pos.m.sell', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6v12m6-6H6"/>'],
        ...
        ['key' => 'more', 'label' => 'Settings', 'route' => 'pos.m.settings', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"/>'],
    ];
@endphp
<nav class="pos-m-nav">
    @foreach($navItems as $item)
        <a href="{{ route($item['route']) }}" class="pos-m-nav-item {{ $active === $item['key'] ? 'pos-m-nav-item--active' : '' }}">
            <svg class="pos-m-nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">{!! $item['icon'] !!}</svg>
            <span class="pos-m-nav-label">{{ $item['label'] }}</span>
        </a>
    @endforeach
</nav>
