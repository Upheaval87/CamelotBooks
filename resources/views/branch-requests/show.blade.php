                                                    ...
                                                    <td class="text-right whitespace-nowrap">
                                                        @if($payment->status === 'pending' && $canConfirmPayment)
                                                            ...
                                                            <form id="reject-{{ $payment->id }}" method="POST" action="{{ route('branch-requests.payments.reject', [$branchRequest, $payment]) }}" class="hidden mt-2">
                                                                @csrf
                                                                <input type="text" name="reason" placeholder="Rejection reason" required class="mt-1 block w-full rounded-xl border-shell bg-white/80 px-3 py-2 text-xs text-gray-900 focus:border-[rgba(18,143,142,.55)] focus:ring-[3px] focus:ring-[rgba(18,143,142,.15)] focus:outline-none" />
