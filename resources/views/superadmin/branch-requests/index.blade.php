                        ...
                        @forelse($requests as $r)
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-[18px] align-middle">
                                    ...
                                    @if($r->branch_code)
                                        <span class="mt-1 block text-[0.893rem] text-gray-400">{{ $r->branch_code }}</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-5 py-[18px] align-middle">
                                    @if($r->quotation)
                                        ...
                                        <span class="mt-1 block text-[0.893rem] text-gray-400">{{ number_format($r->quotation->total, 2) }} {{ $r->quotation->currency_code }}</span>
                                    @else
                                ...
                                <td class="px-5 py-[18px] text-center align-middle">
                                    <a href="{{ route('superadmin.companies.branch-requests.show', [$r->company_id, $r->id]) }}" class="inline-flex items-center gap-1.5 rounded-[10px] border border-gold-600/35 bg-gradient-to-b from-[#F4FBFB] to-[#DFF7F6] px-4 py-2 text-[0.929rem] font-bold text-gold-700 shadow-edit transition hover:-translate-y-px hover:border-gold-600/55 hover:text-gold-800 hover:shadow-edit-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold-500">
