        ...
        <div class="fr-head">
            ...
            <div class="fr-actions">
                <div x-data="{ open: false }" @click.away="open = false" style="position:relative;display:inline-block">
                    <button type="button" class="fr-btn fr-btn-ghost fr-btn-sm" @click="open = !open">Schedule Payments ▾</button>
                    <div x-show="open" x-transition style="position:absolute;right:0;top:100%;z-index:50;background:#fff;border:1px solid #e5e7eb;border-radius:8px;min-width:200px;box-shadow:0 4px 12px rgba(0,0,0,.1);margin-top:4px">
                        <a href="{{ route('accounting.reports.vendor-statement') }}" style="display:block;padding:8px 16px;color:#374151;text-decoration:none;font-size:.8125rem">Vendor Statement Report</a>
                        <a href="{{ route('accounting.vendor-payments.create') }}" style="display:block;padding:8px 16px;color:#374151;text-decoration:none;font-size:.8125rem;border-top:1px solid #e5e7eb">Make Vendor Payment</a>
                    </div>
                </div>
                ...
                <a href="{{ route('accounting.aging.ap-detail', request()->query()) }}" class="fr-btn fr-btn-ghost fr-btn-sm">View Detail</a>
            </div>
        </div>

        <form method="GET" action="{{ route('accounting.aging.ap-summary') }}" class="fr-filters">
            <div class="fr-f">
            ...
            <div style="display:flex;gap:.5rem">
                ...
                <a href="{{ route('accounting.aging.ap-summary') }}" class="fr-btn fr-btn-ghost fr-btn-sm">Clear</a>
            </div>
        </form>
                    ...
                    <tbody>
                        @forelse($vendors as $row)
                            <tr>
                                ...
                                <td class="r">
                                    @if(($row['days_1_30'] ?? 0) > 0)
                                        <span class="fr-badge fr-badge-amber"><span class="fr-badge-dot"></span>{{ format_number($row['days_1_30']) }}</span>
                                    @else
                                        {{ format_number($row['days_1_30']) }}
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" style="text-align:center;padding:2rem 1rem;color:var(--faint,#8aa5a7)">No outstanding bills found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                    <tfoot>
