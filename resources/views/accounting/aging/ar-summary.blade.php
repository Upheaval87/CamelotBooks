    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('A/R Aging Summary') }}
            </h2>
            <div class="flex gap-2">
                <a href="{{ route('accounting.aging.export-csv', array_merge(request()->query(), ['type' => 'ar'])) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    {{ __('Export CSV') }}
                </a>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex gap-2">
                <a href="{{ route('accounting.aging.ar-summary', request()->query()) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md font-semibold text-xs uppercase tracking-widest shadow-sm">Summary</a>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($customers as $row)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{{ $row['customer_name'] }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-500">No outstanding invoices found.</td>
                                </tr>
                            @endforelse
                        </tbody>
