<x-app-layout>
    <x-slot name="header">{{ __('Edit Pension Scheme') }}</x-slot>
            ...
            <div class="card p-6 text-gray-900">
                ...
                <form action="{{ route('accounting.pension-schemes.update', $scheme) }}" method="POST">
                    ...
                    <div class="flex items-center justify-end mt-6 space-x-3">
                        <x-button variant="ghost" href="{{ route('accounting.pension-schemes.show', $scheme) }}">{{ __('Cancel') }}</x-button>
                        <x-button variant="primary" type="submit">{{ __('Update Scheme') }}</x-button>
                    </div>
                </form>
            </div>
        </div>
