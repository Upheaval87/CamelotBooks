<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Cheques') }}</h2>
            <div class="flex items-center space-x-3">
                <a href="{{ route('accounting.cheques.register') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50">
                    {{ __('Register') }}
                </a>
                <a href="{{ route('accounting.cheques.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                    {{ __('Write Cheque') }}
                </a>
            </div>
        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Cheque #</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($cheques as $cheque)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $cheque->date->format('M d, Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $cheque->bankAccount->name ?? '—' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $cheque->payee }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $cheque->memo ?? '—' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right font-semibold">{{ number_format($cheque->amount, 2) }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-500">No cheques found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
