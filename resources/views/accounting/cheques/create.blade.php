<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Write Cheque') }}</h2>
                ...
                <form method="POST" action="{{ route('accounting.cheques.store') }}">
                    ...
                    <div class="space-y-6">
                            ...
                            <select id="bank_account_id" name="bank_account_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                ...
                                @foreach($bankAccounts as $account)
                                    <option value="{{ $account->id }}" {{ old('bank_account_id') == $account->id ? 'selected' : '' }}>
                                        {{ $account->code }} - {{ $account->name }} (Balance: {{ number_format($account->current_balance, 2) }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="debit_account_id" name="debit_account_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
