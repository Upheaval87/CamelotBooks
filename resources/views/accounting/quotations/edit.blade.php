<x-app-layout>
    ...
    <x-list-header title="Edit Quotation {{ $quotation->quotation_number }}" />
    <div class="pb-12">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-scoped-search-field
                                ...
                                :label="old('customer_name', $quotation->customer->name ?? '')"
                        ...
                        <div><x-input-label for="branch_id" value="{{ __('Branch') }}" /><x-scoped-search-field name="branch_id" entity="branch" search-url="{{ route('accounting.search.entity', ['entity' => 'branch']) }}" :value="old('branch_id', $quotation->branch_id)" :label="old('branch_id', $quotation->branch_id) ? ($branches->firstWhere('id', (int) old('branch_id', $quotation->branch_id))?->name ?? '') : ''" placeholder="{{ __('None') }}" /></div>
                        <div><x-input-label for="cost_center_id" value="{{ __('Cost Center') }}" /><x-scoped-search-field name="cost_center_id" entity="cost-center" search-url="{{ route('accounting.search.entity', ['entity' => 'cost-center']) }}" :value="old('cost_center_id', $quotation->cost_center_id)" :label="old('cost_center_id', $quotation->cost_center_id) ? ($costCenters->firstWhere('id', (int) old('cost_center_id', $quotation->cost_center_id))?->name ?? '') : ''" placeholder="{{ __('None') }}" /></div>
    ...
    <script>
        ...
        function addLine(data) {
            ...
            tr.innerHTML = '<td class="px-4 py-2" style="min-width:220px;">' + picker + '</td>' +
                '<td class="px-4 py-2"><input type="text" name="lines[' + idx + '][description]" value="' + (data ? (data.description || '') : '') + '" readonly class="block w-full border-gray-300 focus:border-gold-500 focus:ring-gold-500 rounded-md shadow-sm text-sm bg-gray-50" /></td>' +
                '<td class="px-4 py-2"><input type="number" name="lines[' + idx + '][quantity]" value="' + (data ? data.quantity : 1) + '" min="0" step="any" class="block w-full border-gray-300 focus:border-gold-500 focus:ring-gold-500 rounded-md shadow-sm text-sm text-right" onchange="updateTotals()" oninput="updateTotals()" /></td>' +
                '<td class="px-4 py-2"><input type="number" name="lines[' + idx + '][unit_price]" value="' + (data ? (data.unit_price || 0) : 0) + '" min="0" step="0.01" readonly class="block w-full border-gray-300 focus:border-gold-500 focus:ring-gold-500 rounded-md shadow-sm text-sm text-right bg-gray-50" onchange="updateTotals()" oninput="updateTotals()" /></td>' +
                '<td class="px-4 py-2"><input type="number" name="lines[' + idx + '][discount]" value="' + (data ? (data.discount || 0) : 0) + '" min="0" max="100" step="0.01" class="block w-full border-gray-300 focus:border-gold-500 focus:ring-gold-500 rounded-md shadow-sm text-sm text-right" onchange="updateTotals()" oninput="updateTotals()" /></td>' +
