<x-app-layout>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <div x-data="customerSearch('{{ old('customer_id', $quotation->customer_id) }}', '{{ old('customer_name', $quotation->customer->name ?? '') }}')" class="relative">
    ...
    <script>
        function customerSearch(selectedId, selectedName) {
            return { query: selectedName || '', selectedId: selectedId || '', results: [], open: false, async search() { if (this.query.length < 1) { this.results = []; this.open = false; return; } const r = await fetch('{{ route("accounting.customers.search") }}?q=' + encodeURIComponent(this.query)); this.results = await r.json(); this.open = true; }, select(c) { this.selectedId = c.id; this.query = c.name; this.open = false; } }
        }
        const products = @json($productsJson);
        const incomeAccounts = @json($incomeAccounts);
        const costCenters = @json($costCenters);
        let lineIndex = 0;
        function onProductSelected(idx, id, item) { var row = document.querySelector('[data-line-idx="' + idx + '"]'); if (!row) return; var descInput = row.querySelector('[name*="[description]"]'); if (descInput) descInput.value = item.description || ''; var priceInput = row.querySelector('[name*="[unit_price]"]'); if (priceInput) priceInput.value = item.sales_price ? parseFloat(item.sales_price).toFixed(2) : '0.00'; var taxInput = row.querySelector('[name*="[tax_rate]"]'); if (taxInput) taxInput.value = item.tax_rate ? parseFloat(item.tax_rate).toFixed(2) : '0.00'; var accSelect = row.querySelector('[name*="[income_account_id]"]'); if (accSelect && item.income_account_id) accSelect.value = item.income_account_id; updateTotals(); }
        function buildProductSearchable(idx, selectedId, selectedName) { var config = { name: 'lines[' + idx + '][product_id]', items: products, valueKey: 'id', labelKey: 'name', searchKeys: ['name', 'sku', 'barcode'], showFields: ['sku', 'sales_price'], preload: selectedId || '', preloadLabel: selectedName || '', onSelectCallback: 'onProductSelect_' + idx, enableAdvancedSearch: true, advancedSearchName: 'product' }; window['onProductSelect_' + idx] = function(id, item) { onProductSelected(idx, id, item); }; return 'searchableSelect(' + JSON.stringify(config) + ')'; }
        function escapeHtml(str) { return str.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }
        function updateTotals() { var subtotal = 0, totalTax = 0; document.querySelectorAll('#lines-body tr').forEach(function(row) { var qty = parseFloat(row.querySelector('[name*="[quantity]"]').value) || 0; var price = parseFloat(row.querySelector('[name*="[unit_price]"]').value) || 0; var discount = parseFloat(row.querySelector('[name*="[discount]"]').value) || 0; var taxRate = parseFloat(row.querySelector('[name*="[tax_rate]"]').value) || 0; var lineSubtotal = (qty * price) * (1 - discount / 100); var lineTax = lineSubtotal * (taxRate / 100); subtotal += lineSubtotal; totalTax += lineTax; row.querySelector('.line-total').textContent = (lineSubtotal + lineTax).toFixed(2); }); document.getElementById('subtotal').textContent = subtotal.toFixed(2); document.getElementById('total-tax').textContent = totalTax.toFixed(2); document.getElementById('grand-total').textContent = (subtotal + totalTax).toFixed(2); }
        function addLine(data) { var tbody = document.getElementById('lines-body'); var idx = lineIndex++; var selectedId = data ? String(data.product_id || '') : ''; var selectedName = data && data.product_id ? (products.find(function(p) { return p.id == data.product_id; }) || {}).name || '' : ''; var xDataAttr = buildProductSearchable(idx, selectedId, selectedName); var accountOptions = incomeAccounts.map(function(a) { return '<option value="' + a.id + '" ' + (data && data.income_account_id == a.id ? 'selected' : '') + '>' + a.code + ' - ' + a.name + '</option>'; }).join(''); var tr = document.createElement('tr'); tr.setAttribute('data-line-idx', idx); tr.innerHTML = '<td class="px-4 py-2" style="min-width:220px;"><div x-data="' + escapeHtml(xDataAttr) + '" class="relative"><input type="hidden" name="lines[' + idx + '][product_id]" :value="selectedId" /><div class="flex"><input type="text" x-model="query" @input.debounce.200ms="filter()" @focus="if(query.length > 0) open = true" @keydown.down.prevent="moveHighlight(1)" @keydown.up.prevent="moveHighlight(-1)" @keydown.enter.prevent="confirmHighlight()" @keydown.escape="open = false" @keydown.tab="open = false" placeholder="Search products..." autocomplete="off" class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-l-md shadow-sm text-sm" /><button type="button" @click="openAdvancedSearch()" class="px-2 bg-gray-50 border border-l-0 border-gray-300 rounded-r-md hover:bg-gray-100 focus:outline-none" title="Advanced Search"><svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg></button></div></div></td>' + '<td class="px-4 py-2"><input type="text" name="lines[' + idx + '][description]" value="' + (data ? (data.description || '') : '') + '" readonly class="block w-full border-gray-300 rounded-md shadow-sm text-sm bg-gray-50" /></td>' + '<td class="px-4 py-2"><input type="number" name="lines[' + idx + '][quantity]" value="' + (data ? data.quantity : 1) + '" min="0" step="any" class="block w-full border-gray-300 rounded-md shadow-sm text-sm text-right" onchange="updateTotals()" oninput="updateTotals()" /></td>' + '<td class="px-4 py-2"><input type="number" name="lines[' + idx + '][unit_price]" value="' + (data ? (data.unit_price || 0) : 0) + '" min="0" step="0.01" readonly class="block w-full border-gray-300 rounded-md shadow-sm text-sm text-right bg-gray-50" onchange="updateTotals()" oninput="updateTotals()" /></td>' + '<td class="px-4 py-2"><input type="number" name="lines[' + idx + '][discount]" value="' + (data ? (data.discount || 0) : 0) + '" min="0" max="100" step="0.01" class="block w-full border-gray-300 rounded-md shadow-sm text-sm text-right" onchange="updateTotals()" oninput="updateTotals()" /></td>' + '<input type="hidden" name="lines[' + idx + '][tax_rate]" value="' + (data ? (data.tax_rate || 0) : 0) + '" />' + '<td class="px-4 py-2"></td>' + '<td class="px-4 py-2"><select name="lines[' + idx + '][income_account_id]" class="block w-full border-gray-300 rounded-md shadow-sm text-sm"><option value="">Select Account</option>' + accountOptions + '</select></td>' + '<td class="px-4 py-2 text-right text-sm font-medium line-total">0.00</td>' + '<td class="px-4 py-2 text-center"><button type="button" onclick="removeLine(this)" class="text-red-600 hover:text-red-900" title="Remove"><svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></td>'; tbody.appendChild(tr); updateTotals(); }
        ...
        var existingLines = @json($existingLines);
