                ...
                <form method="POST" action="{{ route('accounting.customers.store') }}">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="billing_address" name="billing_address" rows="2" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('billing_address') }}</textarea>
                            <x-input-error :messages="$errors->get('billing_address')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="shipping_address" value="{{ __('Shipping Address') }}" />
                            <textarea id="shipping_address" name="shipping_address" rows="2" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('shipping_address') }}</textarea>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="payment_terms" name="payment_terms" class="input mt-1">
                                    ...
                                    <option value="net_90" {{ old('payment_terms') === 'net_90' ? 'selected' : '' }}>Net 90</option>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="payment_terms_days" name="payment_terms_days" type="number" class="mt-1 block w-full" :value="old('payment_terms_days')" min="0" />
                                <x-input-error :messages="$errors->get('payment_terms_days')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="credit_limit" value="{{ __('Credit Limit') }}" />
                                <x-text-input id="credit_limit" name="credit_limit" type="number" step="0.01" class="mt-1 block w-full" :value="old('credit_limit')" min="0" />
