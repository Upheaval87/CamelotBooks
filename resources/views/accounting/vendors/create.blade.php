<x-app-layout>
    ...
    <x-list-header title="{{ __('Create Vendor') }}" />
            ...
            <div class="form-page">
                <div class="form-page-main">
                    <div class="card p-6">
                        <form method="POST" action="{{ route('accounting.vendors.store') }}">
                            ...
                            <x-form.section number="01" :title="__('Vendor Details')" />
                            ...
                            <div class="mt-6">
                                ...
                                <textarea id="billing_address" name="billing_address" rows="2" class="input mt-1">{{ old('billing_address') }}</textarea>
                                <x-input-error :messages="$errors->get('billing_address')" class="mt-2" />
                            </div>

                            <div class="mt-4">
                                <x-input-label for="remit_to_address" value="{{ __('Remit To Address') }}" />
                                <textarea id="remit_to_address" name="remit_to_address" rows="2" class="input mt-1">{{ old('remit_to_address') }}</textarea>
                ...
                <x-form.quick-actions :title="__('Quick Actions')" :groups="[
                    ['label' => __('Create'), 'links' => [
                        ...
                        ['title' => __('New Payment'), 'route' => route('accounting.vendor-payments.create'), 'icon' => 'banknotes'],
                    ]],
                    ['label' => __('View'), 'links' => [
                        ['title' => __('Vendor List'), 'route' => route('accounting.vendors.index'), 'icon' => 'person'],
                    ]],
                ]" />
            </div>
        </div>
