<x-app-layout>
    ...
    <x-list-header title="{{ __('Edit Vendor') }}: {{ $vendor->name }}" />
                    ...
                    <div class="card p-6">
                        <form method="POST" action="{{ route('accounting.vendors.update', $vendor) }}">
                            ...
                            <div class="mt-6">
                                ...
                                <textarea id="billing_address" name="billing_address" rows="2" class="input mt-1">{{ old('billing_address', $vendor->billing_address) }}</textarea>
                                <x-input-error :messages="$errors->get('billing_address')" class="mt-2" />
                            </div>

                            <div class="mt-4">
                                <x-input-label for="remit_to_address" value="{{ __('Remit To Address') }}" />
                                <textarea id="remit_to_address" name="remit_to_address" rows="2" class="input mt-1">{{ old('remit_to_address', $vendor->remit_to_address) }}</textarea>
