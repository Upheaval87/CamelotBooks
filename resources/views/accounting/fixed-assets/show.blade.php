<x-app-layout>
    <x-slot name="header">{{ __('Fixed Asset Detail') }}</x-slot>

    <div class="flex items-center justify-end gap-2 mb-4">
        @if($asset->status === 'draft')
            <x-button variant="primary" href="{{ route('accounting.fixed-assets.edit', $asset) }}">{{ __('Edit') }}</x-button>
        ...
        @endif
        <x-button variant="ghost" href="{{ route('accounting.fixed-assets.schedule', $asset) }}">{{ __('Depreciation Schedule') }}</x-button>
        <x-button variant="ghost" href="{{ route('accounting.fixed-assets.index') }}">{{ __('Back to Assets') }}</x-button>
    </div>
