    ...
    <script>
        ...
        function addLine(data) {
            ...
            tr.innerHTML =
                ...
                '</td>' +
                '<td class="px-4 py-2">' +
                    '<input type="number" name="lines[' + idx + '][unit_cost]" value="' + (data ? (data.unit_cost || 0) : 0) + '" min="0" step="0.01" class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm text-right" onchange="updateTotals()" oninput="updateTotals()" required />' +
                '</td>' +
                '<td class="px-4 py-2">' +
