<x-app-layout>
    <x-slot name="header">{{ __('Transfers & Adjustments') }}</x-slot>

    <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 py-6">
        <div class="inv-hdr">
            <div>
                <h1 class="inv-hdr-t">{{ __('Transfers & Adjustments') }}</h1>
                <p class="inv-hdr-sub">{{ __('Track movement between warehouses and adjustment history.') }}</p>
            </div>
        ...
        <div class="inv-tabs">
            ...
            <a href="{{ route('accounting.invsetup.transfers') }}" class="inv-tab inv-tab-on">{{ __('Transfers & Adjustments') }}</a>
            ...
            <div class="inv-main">
                <div class="inv-sgrid inv-sgrid-3">
                    <div class="inv-sbox">
                        ...
                        <div class="inv-sbox-lbl">{{ __('Total Transfers') }}</div>
                        <div class="inv-sbox-v">{{ $transfers->total() }}</div>
                    </div>
                    <div class="inv-sbox">
                        <div class="inv-sbox-ic inv-sbox-ic-amber">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                        </div>
                        <div class="inv-sbox-lbl">{{ __('Pending') }}</div>
                        <div class="inv-sbox-v">{{ $statusCounts->get('pending', 0) }}</div>
                    </div>
                    <div class="inv-sbox">
                        ...
                        <div class="inv-sbox-v">{{ $statusCounts->get('completed', 0) }}</div>
                    </div>
                </div>
                        ...
                        <div class="inv-tbl-wrap">
                            <table class="inv-tbl">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="inv-mono">{{ $transfer->transfer_number ?? '—' }}</td>
                                        <td>{{ $transfer->created_at?->format('M d, Y') ?? '—' }}</td>
                                        <td>{{ $transfer->product?->name ?? '—' }}</td>
                                        <td class="inv-numr">{{ $transfer->quantity ?? '—' }}</td>
                                        <td>
                                            <span class="inv-pill inv-pill-{{ ($transfer->status ?? '') === 'completed' ? 'act' : (($transfer->status ?? '') === 'cancelled' ? 'inact' : 'wip') }}">
                                                {{ ucfirst($transfer->status ?? 'pending') }}
                                            </span>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr><td colspan="5" class="inv-empty">{{ __('No transfers found.') }}</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="inv-rail">
                <div class="inv-rail-card">
                    <div class="inv-rail-sec">
                        ...
                        <a href="{{ route('accounting.invsetup.transfers') }}" class="inv-rail-item inv-rail-item-on">
