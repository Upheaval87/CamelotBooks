<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('VAT / Tax Return') }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <x-toolbar class="mb-6">
                ...
                <button type="button" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-transparent border border-atlas-navy/20 text-atlas-navy text-sm font-medium rounded-md hover:bg-gray-50 transition-colors">
                ...
                <x-slot name="right">
                    <button type="button" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-transparent border border-atlas-amber text-atlas-navy text-sm font-medium rounded-md hover:bg-atlas-amber/10 transition-colors">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($taxAccounts as $acct)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $acct->code }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $acct->name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right font-medium">{{ format_money($acct->current_balance ?? 0) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
