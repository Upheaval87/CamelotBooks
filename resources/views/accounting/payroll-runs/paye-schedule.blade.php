...
<head>
    ...
    <title>{{ __('PAYE Schedule') }} — {{ $run->run_number }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h1>{{ __('PAYE Remittance Schedule') }}</h1>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th class="text-right">{{ __('PAYE Amount') }}</th>
                </tr>
            </thead>
            <tbody>
                ...
                @forelse($run->items as $index => $item)
                    @if($item->paye > 0)
                        @php $totalPaye += $item->paye; @endphp
                        <tr>
                            <td>{{ $index + 1 }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="3" style="text-align: center; padding: 30px; color: #888;">
                            {{ __('No PAYE deductions found for this run.') }}
                        </td>
                    </tr>
                @endforelse
                @if($totalPaye > 0)
