    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('accounting.bank-accounts.transfer') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    {{ __('Transfer') }}
                </a>
                <a href="{{ route('accounting.bank-accounts.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            ...
                            @forelse($bankAccounts as $bankAccount)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                        ...
                                        <a href="{{ route('accounting.bank-reconciliation.index', ['bank_account_id' => $bankAccount->id]) }}" class="text-indigo-600 hover:text-indigo-900">Reconcile</a>
                                        <a href="{{ route('accounting.bank-accounts.manual-transaction', ['bank_account_id' => $bankAccount->id]) }}" class="text-indigo-600 hover:text-indigo-900">Manual Transaction</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
