...
<head>
    ...
    @php
        $cur = $order->currency ?: 'MWK';
        $companyName = $order->company?->name ?? config('app.company_name', 'CamelotBooks');
    ...
    <div class="canvas">
        ...
        <div class="qpdf-parties">
            ...
            <div class="qpdf-detail">
                <div class="d"><span class="l">{{ __('Branch') }}</span><span class="v">{{ $order->branch?->name ?? '—' }}</span></div>
                <div class="d"><span class="l">{{ __('Prepared By') }}</span><span class="v">{{ $prepared }}</span></div>
                <div class="d"><span class="l">{{ __('Cost Centre') }}</span><span class="v">{{ $order->costCenter?->name ?? '—' }}</span></div>
            </div>
        </div>
        ...
        <div class="qpdf-li">
            <table>
                ...
                <tbody>
                    @forelse($order->lines as $idx => $line)
                        <tr>
        ...
        <div class="qpdf-totrow">
            <div></div>
            <div class="totals">
                <div class="trow"><span>{{ __('Subtotal') }}</span><span class="v">{{ $cur }} {{ format_number($order->amount) }}</span></div>
                <div class="trow"><span>{{ __('Tax') }}</span><span class="v">{{ $cur }} {{ format_number($order->tax_total) }}</span></div>
                <div class="gt"><span class="gl">{{ __('Grand Total') }}</span><span class="gv">{{ $cur }} {{ format_number($order->total) }}</span></div>
            </div>
        </div>
        ...
        <div class="qpdf-lower">
            <div class="qpdf-terms">
                @if($order->memo)
                    ...
                    <div class="qpdf-blk">{{ $order->memo }}</div>
                @endif
                <div class="qpdf-h4">{{ __('Terms') }}</div>
                <div class="qpdf-blk"><ul>
                    <li>{{ __('Goods to be delivered on the expected delivery date.') }}</li>
                    <li>{{ __('Sales Order № must be referenced on delivery.') }}</li>
                </ul></div>
            </div>
