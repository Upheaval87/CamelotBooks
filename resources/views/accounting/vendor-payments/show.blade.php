    ...
    <div class="suite ex-suite stage pb-6">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="sticky-head">
                <div>
                    <h1>{{ __('Vendor Payment') }} <span class="mono-chip">{{ $payment->payment_number }}</span></h1>
                    ...
                    <section class="card">
                        <div class="card-sec" style="padding-bottom:8px">
                            ...
                            <div class="tab-panel" id="tab-overview">
                                <div class="g4">
                                    <div class="field"><label>{{ __('Vendor') }}</label><a href="{{ route('accounting.vendors.show', $payment->vendor) }}" style="font-weight:600;color:var(--sec,#128F8E)">{{ $payment->vendor->name ?? '—' }}</a></div>
                            ...
                            <div class="tab-panel" id="tab-allocations" style="display:none">
                                <div class="li-wrap">
                                    <table>
                                        <thead><tr>
                                            <th style="width:26%">{{ __('Bill #') }}</th>
                                            <th style="width:24%">{{ __('Date') }}</th>
                                            <th class="num" style="width:25%">{{ __('Bill Amount') }} ({{ $cs }})</th>
                                            <th class="num" style="width:25%">{{ __('Allocated') }} ({{ $cs }})</th>
                                        </tr></thead>
                                        <tbody>
                                            @forelse($payment->allocations as $allocation)
                                                <tr>
                                                    <td><a href="{{ route('accounting.bills.show', $allocation->bill) }}" class="link mono">{{ $allocation->bill->bill_number ?? '—' }}</a></td>
                                                    <td style="font-weight:600;color:var(--ink,#0B2A2D)">{{ $allocation->bill?->bill_date?->format('M d, Y') ?? '—' }}</td>
                                                    <td class="numr">{{ format_number($allocation->bill?->amount ?? 0) }}</td>
                ...
                <aside class="railsum">
                    <section class="card">
                        <div class="rail-sec">
                            ...
                            <div class="gt"><span class="l">{{ __('Total Paid') }}</span><span class="v">{{ $cs }}{{ format_number($paidTotal) }}</span></div>
                        </div>
                        <div class="rail-sec">
                            ...
                            <div class="vlist">
                                ...
                                <a href="{{ route('accounting.vendor-payments.create', ['vendor_id' => $payment->vendor_id]) }}" class="vitem"><span class="ic"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 4v16m8-8H4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg></span>{{ __('New Payment') }}</a>
                                <a href="{{ route('accounting.vendors.show', $payment->vendor) }}" class="vitem"><span class="ic"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="9" cy="8" r="3.5" stroke="currentColor" stroke-width="2"/><path d="M2.5 20c1.2-3.5 4-5 6.5-5s5.3 1.5 6.5 5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg></span>{{ __('View Vendor') }}</a>
                                <a href="{{ route('accounting.vendors.index') }}" class="vitem"><span class="ic"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M10 19l-7-7 7-7M3 12h18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>{{ __('All Vendors') }}</a>
                            </div>
                        </div>
                    </section>
                </aside>
