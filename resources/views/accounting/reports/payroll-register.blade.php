...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <x-list-header title="Payroll Register" />
    <form method="GET" class="bg-white shadow-sm sm:rounded-lg p-4 mb-6 flex gap-4 items-end">
        @csrf
        <div><label class="block text-sm font-medium text-gray-700">Payroll Run ID</label><input type="text" name="payroll_run_id" value="{{ request('payroll_run_id') }}" placeholder="Payroll Run ID" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm"></div>
        ...
        <table class="datasheet">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="numeric">{{ format_number($row['basic_pay']) }}</td>
                    <td class="numeric">{{ format_number($row['total_allowances']) }}</td>
                    <td class="numeric">{{ format_number($row['gross_pay']) }}</td>
                    ...
                    <td class="figure px-4 py-2 text-sm text-right font-semibold">{{ format_number($row['net_pay']) }}</td>
