...
<div class="max-w-8xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <x-list-header title="{{ __('Sales Receipts Cashbook') }}" description="{{ __('Cashbook-style listing of posted sales receipts for the selected period.') }}" />
    ...
    <form method="GET" class="q2-card q2-filters mt-4">
        ...
        <div class="q2-filters-actions">
            ...
            <a href="{{ route('accounting.reports.sales-receipts.cashbook') }}" class="q2-btn q2-btn--ghost">{{ __('Reset') }}</a>
        </div>
    </form>
    ...
    <div class="q2-statgrid mt-4">
        <div class="q2-stat">
            ...
            <div class="q2-stat-meta">
                ...
                <span class="q2-stat-val">{{ number_format(count($rows)) }}</span>
        ...
        @if(count($rows) > 0)
            <div class="q2-tbl-wrap" style="border:none;border-radius:0">
                <table class="q2-tbl">
                    ...
                    <tbody>
                        @foreach($rows as $row)
                            <tr>
                                ...
                                <td class="q2-mono">{{ $row['receipt_number'] }}</td>
                                <td style="font-weight:600;color:var(--ink,#0B2A2D)">{{ $row['customer_name'] }}</td>
                                <td>{{ $row['method'] }}</td>
                                <td class="q2-mono">{{ $row['reference'] }}</td>
                    ...
                    <tfoot>
                        <tr>
                            <td class="q2-lbl" colspan="5">{{ __('Total') }}</td>
