<x-app-layout>
    <x-list-header title="{{ __('Record Customer Payment') }}" />
        ...
        function loadInvoices(customerId) {
            ...
            tbody.innerHTML = invoices.map(inv => `
                <tr data-invoice-id="${inv.id}" data-balance="${inv.balance_due}">
                    ...
                    <td class="numeric">
                        <input type="number" name="allocations[${inv.id}]" class="allocation-input block w-32 border-gray-300 focus:border-gold-500 focus:ring-gold-500 rounded-md shadow-sm text-sm text-right" min="0" max="${inv.balance_due}" step="0.01" value="0" oninput="updateAllocationTotals()" />
                    </td>
                </tr>
            `).join('');
            autoAllocate();
