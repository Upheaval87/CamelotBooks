<x-app-layout>
    @php
        ...
        $selectedCustomer = old('customer_id') ? $customers->firstWhere('id', (int) old('customer_id')) : ($preselectCustomer ?? null);
    @endphp
    ...
    <div class="suite pb-6">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="sticky-head">
                <div>
                    <h1>{{ __('Record Customer Payment') }}</h1>
                    <div class="sub">{{ __('Receive money from a customer and apply it to open invoices.') }}</div>
                </div>
                <div class="tbtns">
                    <a href="{{ route('accounting.customers.index') }}" class="btn ghost sm">{{ __('Cancel') }}</a>
                    <button type="submit" form="cp-form" class="btn cta">{{ __('Record Payment') }}</button>
                </div>
            </div>

            <form method="POST" action="{{ route('accounting.customer-payments.store') }}" id="cp-form" novalidate>
                @csrf
                    ...
                    <div class="flex flex-col gap-5 min-w-0">
                        ...
                        <section class="card card-sec">
                            ...
                            <div class="g4">
                                ...
                                <div class="field sp2">
                                    <label for="bank_account_id">Deposit To <span class="req">*</span></label>
                                    <x-scoped-search-field
                                    ...
                                    <tbody class="divide-y divide-gray-200" id="alloc-body">
                                        <tr id="no-alloc-row">
                                            <td colspan="5" style="padding:28px 16px;text-align:center;color:var(--muted,#5f7476)">
                                                {{ __('Select a customer to load open invoices.') }}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                    ...
                    <aside class="railsum">
                        <section class="card">
                            ...
                            <div class="rail-sec">
                                ...
                                <div class="vlist">
                                    <a href="{{ route('accounting.invoices.create') }}" class="vitem"><span class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M16 13H8M16 17H8M10 9H8"/></svg></span>{{ __('New Invoice') }}</a>
                                    ...
                                    <a href="{{ route('accounting.customers.index') }}" class="vitem"><span class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 19l-7-7 7-7M3 12h18"/></svg></span>{{ __('Customers List') }}</a>
                                </div>
                            </div>
                        </section>
                    </aside>
    ...
    <script>
        ...
        function loadInvoices() {
            ...
            if (!invoices.length) {
                tbody.innerHTML = '<tr><td colspan="5" style="padding:28px 16px;text-align:center;color:var(--muted,#5f7476)">No open invoices for this customer.</td></tr>';
                updateAllocationTotals();
                return;
            }
            tbody.innerHTML = invoices.map((invoice, i) => `
                <tr data-idx="${i}" data-balance="${invoice.balance_due}">
                    <td class="mono">${invoice.invoice_number}</td>
                    <td>${invoice.invoice_date ?? '—'}</td>
                    <td class="numr">${Number(invoice.amount).toFixed(2)}</td>
                    <td class="numr">${Number(invoice.balance_due).toFixed(2)}</td>
                    <td style="text-align:right">
                        <input type="hidden" name="allocations[${i}][invoice_id]" value="${invoice.id}" />
                        <input type="number" name="allocations[${i}][amount]" class="input alloc-input" min="0" max="${invoice.balance_due}" step="0.01" value="0" style="text-align:right" oninput="updateAllocationTotals()" />
                    </td>
                </tr>
            `).join('');
            autoAllocate();
