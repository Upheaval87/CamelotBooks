<x-app-layout>
    @php
        ...
        $fboxes = [
            ['key' => '', 'label' => __('Total'), 'icon' => 'M3 7a2 2 0 012-2h14a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V7z', 'cls' => 't-ink', 'val' => number_format($stats['total'])],
            ['key' => 'draft', 'label' => __('Draft'), 'icon' => 'M12 8a4 4 0 100 8 4 4 0 000-8zm-8 4a8 8 0 1116 0 8 8 0 01-16 0z', 'cls' => 't-gray', 'val' => number_format(($stats['by_status']['draft'] ?? 0))],
            ['key' => 'posted', 'label' => __('Posted'), 'icon' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', 'cls' => 't-teal', 'val' => number_format(($stats['by_status']['posted'] ?? 0))],
            ['key' => 'applied', 'label' => __('Applied'), 'icon' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', 'cls' => 't-mint', 'val' => number_format(($stats['by_status']['applied'] ?? 0))],
            ['key' => 'void', 'label' => __('Void'), 'icon' => 'M6 18L18 6M6 6l12 12', 'cls' => 't-gray', 'val' => number_format(($stats['by_status']['void'] ?? 0))],
        ];
    @endphp
    ...
    <div class="suite pb-6">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="page-head">
                <div>
                    <h1>{{ __('Vendor Credits') }}</h1>
                    <p class="sub">{{ __('Record credits received from vendors for returns and adjustments.') }}</p>
                </div>
                <div class="tbtns">
                    <a href="{{ route('accounting.vendor-credits.create') }}" class="btn btn-cta">＋ {{ __('Create Vendor Credit') }}</a>
                </div>
            </div>
            ...
            <div class="toolbar">
                <div class="search">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Vendor, number, reference…" autocomplete="off"
                           form="vc-list-form" oninput="debounceVcSearch(this)" class="input" />
                </div>
                <select name="status" form="vc-list-form" class="input" style="width: 13rem" onchange="this.form.submit()">
            ...
            <form id="vc-list-form" method="GET" action="{{ route('accounting.vendor-credits.index') }}">
            ...
            <div class="statgrid statgrid--5" style="margin-top:16px">
                @foreach($fboxes as $box)
                    <a href="{{ route('accounting.vendor-credits.index', $box['key'] ? ['status' => $box['key']] : []) }}"
                       class="fbox @if($activeStatus === $box['key']) on @endif">
                        <span class="t {{ $box['cls'] }}"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="{{ $box['icon'] }}" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>
            ...
            <div class="shell" style="margin-top:20px">
                <div>
                    <div class="card">
                            ...
                            <table>
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width:18%">{{ __('Vendor') }}</th>
                                        <th style="width:10%">{{ __('Date') }}</th>
                                        <th style="width:12%">{{ __('Reference Bill') }}</th>
                                ...
                                <tbody>
                                    @forelse($vendorCredits as $vendorCredit)
                                        <tr>
                                            <td class="mono"><a href="{{ route('accounting.vendor-credits.show', $vendorCredit) }}" class="link">{{ $vendorCredit->credit_note_number }}</a></td>
                                            <td style="font-weight:600;color:var(--deep-3,#0A2E32)">{{ $vendorCredit->vendor->name ?? '—' }}</td>
                                            <td>{{ $vendorCredit->credit_note_date?->format('M d, Y') ?? '—' }}</td>
                                            <td class="em">{{ $vendorCredit->bill->bill_number ?? '—' }}</td>
                                            <td class="numr">{{ format_number($vendorCredit->total) }}</td>
                                            <td class="numr">{{ format_number($vendorCredit->amount_applied) }}</td>
                                            <td class="numr">{{ format_number($vendorCredit->available) }}</td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="row-act">
                                                    <a href="{{ route('accounting.vendor-credits.show', $vendorCredit) }}" class="ibtn" title="{{ __('View') }}" aria-label="{{ __('View') }}">
                                                    ...
                                                    @if($vendorCredit->status === 'draft')
                                                        <form method="POST" action="{{ route('accounting.vendor-credits.post', $vendorCredit) }}" class="inline">
                                                            @csrf
                                                            <button type="submit" class="ibtn" title="{{ __('Post') }}" aria-label="{{ __('Post') }}" onclick="return fbConfirmButton(event, 'Post this vendor credit?', { type: 'action' })">
                                    ...
                                    @empty
                                        <tr><td colspan="9" class="empty">{{ __('No vendor credits found.') }}</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
                        @if($vendorCredits->hasPages())
                            <div class="pagi">
                                <span class="t">{{ __('Showing') }} {{ $vendorCredits->firstItem() }}–{{ $vendorCredits->lastItem() }} {{ __('of') }} {{ $vendorCredits->total() }}</span>
                                <div class="pg">
                                    <a href="{{ $vendorCredits->appends(request()->query())->previousPageUrl() }}" class="pgbtn @if($vendorCredits->onFirstPage()) is-disabled @endif" aria-label="{{ __('Previous') }}">‹</a>
                                    @foreach ($vendorCredits->appends(request()->query())->getUrlRange(1, $vendorCredits->lastPage()) as $page => $url)
                                        @if ($page == $vendorCredits->currentPage())
                                            <span class="pgbtn cur">{{ $page }}</span>
                                        @else
                                            <a href="{{ $url }}" class="pgbtn">{{ $page }}</a>
                                        @endif
                                    @endforeach
                                    <a href="{{ $vendorCredits->appends(request()->query())->nextPageUrl() }}" class="pgbtn @if(!$vendorCredits->hasMorePages()) is-disabled @endif" aria-label="{{ __('Next') }}">›</a>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                <aside class="railsum">
                    <div class="card rail-sec">
                        ...
                        <div class="vlist">
                            <a href="{{ route('accounting.vendor-credits.index') }}" class="vitem @if(!$activeStatus) on @endif">
                                ...
                                <span>{{ __('All Vendor Credits') }}</span>
                            </a>
                            <a href="{{ route('accounting.vendor-credits.index', ['status' => 'draft']) }}" class="vitem @if($activeStatus === 'draft') on @endif">
                            ...
                            <a href="{{ route('accounting.vendor-credits.index', ['status' => 'posted']) }}" class="vitem @if($activeStatus === 'posted') on @endif">
                            ...
                            <a href="{{ route('accounting.vendor-credits.index', ['status' => 'applied']) }}" class="vitem @if($activeStatus === 'applied') on @endif">
                                <span class="ic"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>
                        ...
                        <div class="vlist">
                            ...
                            <a href="{{ route('accounting.reports.purchases-by-vendor') }}" class="vitem">
                                <span class="ic"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 3v2l6 7v7l3 1 3-1v-7l6-7V3H3z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>
    ...
    <script>
        ...
        function debounceVcSearch(input) {
            clearTimeout(vcSearchTimer);
            vcSearchTimer = setTimeout(() => {
                const form = document.getElementById('vc-list-form');
