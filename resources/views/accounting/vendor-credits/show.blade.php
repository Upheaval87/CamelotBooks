    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Vendor Credit') }} #{{ $vendorCredit->vendor_credit_number }}
            </h2>
            <div class="flex items-center space-x-3">
                ...
                @if($vendorCredit->status !== 'void')
                    <form method="POST" action="{{ route('accounting.vendor-credits.void', $vendorCredit) }}" class="inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150" onclick="return confirm('Are you sure you want to void this vendor credit?')">
                            {{ __('Void') }}
                        </button>
                    </form>
                @endif
                    ...
                    @if($vendorCredit->bill)
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">
                                <a href="{{ route('accounting.bills.show', $vendorCredit->bill) }}" class="text-indigo-600 hover:text-indigo-900">
                ...
                <div class="flex justify-end mt-4">
                    <div class="w-64 space-y-2">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="text-gray-900">{{ number_format($vendorCredit->subtotal, 2) }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Tax:</span>
                            <span class="text-gray-900">{{ number_format($vendorCredit->tax_total, 2) }}</span>
                        </div>
                        <div class="flex justify-between text-sm font-semibold border-t pt-2">
                            <span class="text-gray-800">Total:</span>
                            <span class="text-gray-900">{{ number_format($vendorCredit->total, 2) }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Applied:</span>
                            <span class="text-gray-900">{{ number_format($vendorCredit->amount_applied, 2) }}</span>
                        </div>
                        <div class="flex justify-between text-sm font-bold border-t pt-2">
                            <span class="text-gray-800">Available:</span>
                            <span class="text-gray-900">{{ number_format($vendorCredit->available, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
