...
<body onload="window.print()">
    @php
        ...
        $tax = $invoice->lines->sum(fn ($l) => (float) $l->tax_amount);
        $discountTotal = $invoice->lines->sum(fn ($l) => (($l->unit_price * $l->quantity) * (($l->discount ?? 0) / 100)));
        $total = $subtotal + $tax;
        $cs = \App\Models\SystemSetting::getValue('currency', 'currency_symbol', session('current_company_id'), 'MK');
    ...
    <div class="stage-pdf">
        <div class="paper">
            ...
            <div class="parties">
                <div class="party">
                    <div class="p-l">{{ __('Billed To') }}</div>
                    <div class="name">{{ $customer?->name ?? '—' }}</div>
                    <div class="rows">
                        {{ __('Attn') }}: {{ $customer?->name ?? '—' }}<br>
                        {{ $customer?->address ?: '—' }}<br>
            ...
            <div class="p-totals">
                <div></div>
                <div>
                    <div class="trow"><span>{{ __('Subtotal') }}</span><span class="v">{{ format_number($subtotal) }}</span></div>
                    @if($discountTotal > 0)
                        <div class="trow"><span>{{ __('Discount') }}</span><span class="v">{{ format_number($discountTotal) }}</span></div>
                    @endif
                    @if($tax > 0)
                        <div class="trow"><span>{{ __('Tax') }}</span><span class="v">{{ format_number($tax) }}</span></div>
                    @endif
                    <div class="p-gt"><span class="l">{{ __('Total') }}</span><span class="v">{{ $cs }} {{ format_number($total) }}</span></div>
                    <div class="p-words">{{ __('Amount in words') }}: {{ $words($total) }}</div>
                </div>
            </div>
            ...
            <div class="p-lower">
                <div class="p-terms">
                    ...
                    <p>
                        @if($invoice->memo)
                            {{ $invoice->memo }}<br><br>
                        @endif
                        {{ __('Payment is due by the due date above.') }}<br>
                        {{ __('Please quote the invoice number in all correspondence.') }}<br>
                ...
                <div class="p-pay">
                    ...
                    <div class="rows">
                        {{ $company?->bank_name ?: 'National Bank — Head Office' }}<br>
                        {{ __('Account Name') }}: {{ $companyName }}<br>
                        {{ __('Account Number') }}: {{ $company?->bank_account ?: '0000000000' }}<br>
                        {{ __('Branch') }}: {{ $company?->bank_branch ?: '—' }}
                    </div>
                </div>
            </div>

            <div class="p-auth">
                <h4 class="blk">{{ __('Authorised By') }}</h4>
