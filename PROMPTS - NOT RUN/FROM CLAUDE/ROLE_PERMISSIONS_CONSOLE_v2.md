ROLE PERMISSIONS CONSOLE — REDESIGNED PERMISSIONS EDITOR — FULL IMPLEMENTATION SPEC — v2
...
1.1 `code` is set once at role creation and is immutable thereafter — only `name` and
`description` may be edited later. Never let a save regenerate or allow editing `code`,
since it may be referenced as a hardcoded key elsewhere in the system (e.g.
`hasRole('finance_manager')`); renaming it silently would break those references.
...
2.2b DEACTIVATE ROLE: each role card gets a Deactivate/Reactivate action (in addition to
Duplicate) that toggles `roles.active`. Deactivating a role does not delete
`role_permissions` (grants are preserved for reactivation) but immediately blocks
`Authz::can` from honoring that role for any user holding it via `user_roles` — treat an
inactive role as denying, not merely hiding. Show a confirmation before deactivating a
role that still has `user_roles` assigned ("{n} users currently hold this role — they
will lose the access it grants immediately"). There is no hard delete in this console —
only deactivate/reactivate — to avoid orphaning `user_roles`/`role_permissions`/audit
history; a genuine delete is an out-of-scope database operation, not a console action.
...
3.2b CONCURRENCY GUARD: the baseline snapshot taken at load (§3.1) is stamped with the
role's current `updated_at`/version at that moment. Before applying the diff, re-check
that stamp against the role's current value in the same transaction; if it has changed
since load (another admin saved in the meantime), abort the save, do not apply the diff,
and surface "This role's permissions changed since you loaded them — reload to see the
latest and reapply your changes" rather than silently overwriting the other admin's save.
...
3.7 Role card click selects role (loads its grants as baseline). If there are unsaved
changes on the currently-open role, switching cards must show an explicit confirm dialog
with three real options — [Save & Switch] [Discard & Switch] [Cancel] — not a vague
"per app convention" default; do not silently discard or silently switch without asking.
3.8 UNSAVED-CHANGES NAVIGATION GUARD: in addition to §3.7's in-console guard, register a
`beforeunload` handler while `dirty > 0` so a browser refresh, tab close, or navigation
away from the console also warns the admin before unsaved permission edits are lost —
this data is higher-stakes than a typical form, so silent loss on refresh is not
acceptable.
...
4.1 SoD CONFLICT SURFACING: this console is where conflicting grants actually get
introduced (e.g. checking both Create and Approve for the same feature on one role), so
before Save commits, run the change through the central engine's existing SoD conflict
rules (the same ones used for the Authorization Conflicts dashboard). If the resulting
grant set would create a known-conflicting combination, show an inline warning on the
affected row(s)/cell(s) — amber highlight + a short note naming the conflicting actions —
before the admin saves. This is a warning, not necessarily a hard block: allow the admin
to proceed and save anyway (some roles legitimately need it, e.g. a small-team override),
but the warning must be visible before they commit, and the resulting conflict should
still surface normally on the Authorization Conflicts dashboard afterward — don't suppress
it there just because it was acknowledged here.
4.2 AUTHZ:SYNC CATALOG DRIFT: when `authz:sync` adds a new feature/action to the catalog,
it must default to NOT granted (deny) for every existing role, including roles that
previously had "Allow All" clicked on that module — a sync must never silently widen any
role's access. When `authz:sync` removes a feature/action from the catalog, its orphaned
`role_permissions` rows must be cleaned up (deleted) rather than left dangling, and this
cleanup should be logged the same way a manual grant change is audited (§3.2).
...
5.1 SELF-LOCKOUT GUARD: before a save is applied, check whether it would remove the
current user's own ability to access this console (i.e. would strip the last permission
that lets their own role manage roles/permissions). If so, block the save with a clear
error explaining that the change would lock the current admin out of role management,
rather than applying it and leaving no one able to fix it afterward. This check runs in
addition to, not instead of, the SoD warning in §4.1.
...
8.9 CONCURRENCY: two sessions load the same role; session A saves; session B's save is
rejected with a reload prompt rather than silently overwriting A's change (§3.2b).
8.10 SELF-LOCKOUT: attempting to save a change that would strip the current user's own
role-management access is blocked with a clear error (§5.1).
8.11 SoD WARNING: granting a known-conflicting combination (per the engine's existing SoD
rules) on a role shows an inline warning before save, and the conflict still appears on
the Authorization Conflicts dashboard afterward (§4.1).
8.12 ROLE DEACTIVATION: deactivating a role with assigned users shows the user-count
confirmation; Authz::can immediately denies for users holding a deactivated role;
reactivating restores prior grants without needing to re-enter them (§2.2b).
8.13 CATALOG DRIFT: a newly-synced feature/action defaults to ungranted on every existing
role, including one with a prior "Allow All"; a removed feature/action's orphaned
role_permissions rows are cleaned up and logged (§4.2).
8.14 NAVIGATION GUARD: switching roles with unsaved changes shows the three-option
confirm (§3.7); attempting to refresh/close the tab with unsaved changes triggers a
browser warning (§3.8).
REPORT: migration/mapping notes; save-transaction + audit samples; copy-clone proof;
filter matrix; parity confirmation; concurrency-guard confirmation (§3.2b/8.9);
self-lockout guard confirmation (§5.1/8.10); SoD-warning confirmation (§4.1/8.11); role
deactivation confirmation (§2.2b/8.12); authz:sync drift-handling confirmation
(§4.2/8.13); navigation-guard confirmation (§3.7–3.8/8.14); NO SECTION SKIPPED.
