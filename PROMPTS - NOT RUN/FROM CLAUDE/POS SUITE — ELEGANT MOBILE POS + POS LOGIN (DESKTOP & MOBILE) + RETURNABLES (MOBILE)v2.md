POS SUITE — ELEGANT MOBILE POS + POS LOGIN (DESKTOP & MOBILE) + RETURNABLES (MOBILE) — v2
...
SEQUENCING: this spec assumes the desktop POS Centre and desktop Returnables builds are
already complete and stable — it extends the same backend (payment methods, posting
handler, product/customer models, and the BRR/Returnables data model), it does not
reimplement any of them. Do not run this before those builds are finished; if discovery
finds them missing or materially different from what this spec expects, stop and report
rather than guessing. See §0 for the reuse rule this implies, and DELIVERY PHASES near the
end of this prompt for how to sequence this build itself.

==================== 0 · DISCOVERY ====================
0.0 SAFETY NET: before editing auth, checkout, or the posting handler, create a dedicated
branch (or commit current working state if not using git) so this change set can be
reverted as a whole if it interacts badly with existing code. Report the branch/commit
reference used.
...
0.4 CHART OF ACCOUNTS CHECK: confirm that 1320, 2300, 1060/1070/1080/1010, 1100, 4000,
deposit-revenue, and cash-over/short accounts already exist from the prior desktop POS
Centre and Returnables builds — use those exact codes. Do not create new accounts for
anything already established; if any expected account is missing, stop and report rather
than inventing a new code.
0.5 BACKEND REUSE (critical — do not skip): the mobile POS (checkout, products, register,
receipts) and mobile Returnables (intake, BRR receipt, register) must read/write the SAME
underlying models, tables, and posting handler as the desktop builds — not a parallel
schema. Specifically: mobile checkout uses the same sale/receipt model as desktop
pos.checkout; mobile Returnables intake/redemption uses the exact same BRR
(Returnables Receipt) model, table, and status lifecycle as desktop pos.intake/
pos.returnables, so a receipt issued on one channel is redeemable on the other. Confirm
this during discovery before writing any mobile-side model or migration — if a genuinely
new table is unavoidable (e.g. a mobile session/device table), it must reference the
existing shared models by foreign key, not duplicate their data.
0.6 SHARED NUMBERING: receipt numbers (RCP-) and bottle receipt numbers (BRR-) must come
from ONE sequence shared across desktop and mobile channels — confirm the existing
NumberingService/sequence mechanism from the desktop builds is reused as-is, not
reimplemented per-channel, so numbers never collide between a desktop-issued and a
mobile-issued receipt.
...
Open Register / Start Shift. Rate-limit + audit failed attempts; PIN length 4, masked in
UI, and stored hashed at rest (never plaintext, never logged) using the same hashing
approach as the password field — validated by comparing hashes, not stored/compared
values.
...
sessions; audit reset. Verification code expires 10 minutes after issue; an expired or
already-used code is rejected with a clear error and the user must request a new one
(rate-limited to prevent code-request spam).
...
5.1 RELATIONSHIP TO SUPERVISOR APPROVAL: pos.verify re-confirms the CURRENT session
user's own identity — it is a self-reauthentication gate, not the distinct-supervisor
override mechanism defined for desktop over-limit approvals (e.g. blocking an over-credit
sale until a separate, higher-privilege user approves). Use pos.verify as-is for
same-user identity checks (close shift, void, refund, price change). For any action that
specifically requires a DIFFERENT supervisor's approval (over-credit block, discount/
refund limits exceeded by a cashier's own permission tier), reuse pos.verify's keypad/PIN
UI but require it to authenticate as a user with supervisor/admin permission distinct
from the cashier who triggered the block — do not silently let the triggering cashier
clear their own supervisor gate by re-entering their own PIN. Record which user approved,
which user triggered, the action, and a timestamp, matching the audit requirement already
defined for this mechanism on desktop.
...
8.4 COMPLETE SALE: re-checks and re-locks (by receipt number, inside the same transaction
as the journal post) any applied Returnables Receipt's balance immediately before
committing — the earlier §8.2 Apply check was provisional only; two devices applying the
same receipt concurrently must not both be able to complete a sale against it (matching
§14.4's lock rule). Posts journal via existing handler (Dr clearing account(s) for Total
Due incl. extras' deposit; Cr Sales 4000; Cr deposit revenue for extras; returnables
redemption Dr 2300), asserting sum(debit lines) == sum(credit lines) before commit —
abort the whole transaction with a clear error (no partial post, no stock change) if it
doesn't balance. Decrements stock, records served_by = session user, generates receipt
(via the shared numbering sequence, §0.6), navigates to m.pos.receipt.
...
Branch select + Till/Register select at TOP so receipts for the selectable branch+till
can be viewed/reprinted/emailed; payment filter chips (All/Cash/Card/Mobile/Credit/
Returnables); day-grouped list (#№ + tags Returnables/On credit + time·customer·method +
amount); row tap → view/reprint/email. Bottom nav active.
10.1 BRANCH SELECTOR SCOPING (matches the head-office-only rule already applied to
desktop POS/Returnables — do not let mobile bypass it): the Branch selector's option list
itself must be restricted per user — a head-office user sees every branch and may pick
any of them; a non-head-office user sees only their own branch (the selector may simply
show their branch name with no other options, or be hidden entirely if there's nothing to
select between). Never populate the selector with branches the current user isn't
permitted to view, and never trust a branch value submitted from the client without
re-checking it server-side against the user's own visibility.
...
13.1 OFFLINE MODE SCOPE: the "Offline mode" toggle in Preferences is UI/settings-storage
only for this build — it persists the user's preference but does NOT implement actual
offline data capture/sync for mobile in this spec. If mobile should actually function
offline (queue sales locally, sync on reconnect), that needs the same idempotency
mechanism (client-generated UUID per offline sale, checked against already-posted
receipts) defined for desktop offline mode — treat that as a separate, explicitly scoped
follow-up rather than assuming this toggle already does it.
...
14.4 REDEMPTION: occurs ONLY in checkout (§8.2/§8.4). Settlement: covered=min(drinks qty,
BRR bottles); extras=drinks−covered pay full price (drink+deposit); redemption credit=
covered×deposit; leftover credit stays on BRR; Complete Sale reduces BRR bottles/credit;
status→Partial/Redeemed. Apply validates (exists/not void/not expired/balance>0) — this
Apply-time check is provisional only; the authoritative check happens at Complete Sale
under a lock by receipt number (§8.4), so two devices cannot both redeem the same BRR
concurrently. All BRR records created via m.ret.intake use the SAME model/table as
desktop pos.intake (§0.5) — a receipt issued on a phone must be redeemable at the desktop
till and vice versa.
...
short per settings. All via existing journal handler — all accounts confirmed to already
exist per §0.4, none newly created by this spec.
BALANCE GUARD: every journal this spec posts (mobile sale, mobile Returnables intake,
redemption, void, register close over/short) must assert sum(debit lines) ==
sum(credit lines) before commit; abort and surface a clear error rather than posting an
unbalanced entry.
...
served_by + auto receipt with success header; posted journal balances debits=credits
(spot check with a malformed input to confirm rejection); simulate two near-simultaneous
Complete Sale attempts against the same BRR with insufficient combined balance — exactly
one succeeds, the other fails cleanly with no partial post.
19.5 RECEIPTS: Branch+Till selectors filter list; view/reprint/email per row; a
non-head-office user's Branch selector shows only their own branch (no other options
selectable, including via a manually-crafted request); a head-office user's selector
shows all branches.
19.6 REGISTER: cash count variance; X/Z reports; close gated by verify; over/short posted.
19.7 RETURNABLES: intake issues BRR + Dr1320/Cr2300 + stock (via the shared backend, not
a parallel table — verify a BRR issued via m.ret.intake is visible/redeemable at the
desktop pos.checkout and vice versa); BRR branded + fixed table; register statuses + void
reversal; redemption only in checkout.
...
constraint checklist; backend-reuse confirmation (§0.5 — mobile shares desktop's models,
not a parallel schema); shared-numbering confirmation (§0.6); balance-guard confirmation
(§15); redemption-lock timing confirmation (§8.4/§14.4); branch-selector scoping
confirmation (§10.1); pos.verify vs supervisor-approval reconciliation confirmation
(§5.1); PIN-hashing confirmation (§3.3); confirmation NO functional regressions and NO
PAGE SKIPPED.

==================== DELIVERY PHASES ====================
Given the combined scope (auth + full mobile POS + returnables mobile), build and confirm
in this order rather than as one uninterrupted run — pause after each and report per the
REPORT line above before continuing:
Phase A — Auth: pos.login (desktop+mobile), pos.reset, pos.verify (§3–§5), including the
supervisor-approval reconciliation (§5.1) and PIN hashing (§3.3).
Phase B — Mobile POS core: m.pos.home, m.pos.sell, m.pos.checkout, m.pos.receipt (§6–§9),
including the balance guard and redemption-lock timing on Complete Sale (§8.4).
Phase C — Mobile POS supporting pages: m.pos.receipts (with branch-selector scoping,
§10.1), m.pos.register, m.pos.products, m.pos.settings (§10–§13).
Phase D — Returnables mobile: m.ret.intake, m.ret.receipt, m.ret.register (§14),
confirming backend reuse (§0.5) and shared numbering (§0.6) against the desktop
Returnables build.
Do not begin a phase until the prior phase's report has been reviewed.
...
9:41 ●●● · "C Receipts" · "64 today · K486,500" · ES. Selectors: Branch (populated per
§10.1 — a head-office user sees Headquarters / Branch 01 / Branch 02 as shown here; a
non-head-office user would see only their own branch) · Till / Register (All tills /
TILL-01 / TILL-02). Filter chips: All / Cash / Card / Mobile / Credit / Returnables. Day-grouped: Today · 16 Aug · HQ ·
