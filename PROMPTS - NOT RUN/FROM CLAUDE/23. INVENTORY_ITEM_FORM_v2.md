INVENTORY ITEM FORM — ADD + EDIT (ONE SHARED COMPONENT) — FULL IMPLEMENTATION SPEC — v2
...
item form writes master data only; postings flow through existing handlers. Items are
NEVER hard-deleted — an item referenced by any historical transaction, journal line, or
Returnables Receipt must remain in place; Active/Inactive (§2.5) is the only lifecycle
control this form provides.
...
0.3 CHART OF ACCOUNTS CHECK: confirm the account picker's options (Income/Expense/
Inventory/Container Stock account dropdowns) are populated from the SAME chart of
accounts already established in the POS Centre, desktop Returnables, and POS Suite
builds — specifically that 1320 (Returnable Containers), 2300 (Customer Bottle Credits
Liability), and the deposit-revenue account (e.g. 4900) resolve to the exact same
accounts those systems post to. Do not let this form's account picker be sourced from a
separate or newly-created list; if any expected account is missing, stop and report
rather than creating a duplicate under a different code.
...
1.1 LINKED-PAIR DEPOSIT AUTHORITY: when an item has both a `linked_filled_item_id` and a
`linked_empty_item_id` pointing at each other (a filled/empty pair), the FILLED item's
`deposit_value` is authoritative — it's what gets charged at sale and credited at
redemption. The empty-container item's own `deposit_value` field, if the schema stores
one, must either be hidden/disabled in favor of displaying the linked filled item's value
read-only, or validated to always equal it on save (reject a save that would desync
them). Never allow the two sides of a linked pair to hold silently different deposit
values — that would post inconsistent amounts depending on which item's record intake or
redemption happens to read from.
...
2.2b EDITING GL/FINANCIAL FIELDS ON A USED ITEM: if this item already has posted
transactions (any sale, purchase, stock movement, or returnable intake/redemption), show
a non-blocking warning when Income Account, Expense Account, Inventory Account,
Purchase/Sales Price basis, or Costing Method is changed: "This item has existing
transaction history. Changing this account/method only affects future postings — past
journal entries are not altered." Do not block the edit, but ensure it's captured clearly
in the change audit (§3), not just logged silently alongside routine field edits.
...
2.3b OPENING STOCK SCOPE (confirm before building): this form's single Opening
Stock/Warehouse pair sets an initial balance for ONE location only. If this business
operates multiple warehouses/branches needing opening balances entered at creation,
confirm whether that's handled by a separate stock-adjustment/transfer feature (in which
case this form intentionally only covers a single starting location) or whether this
form needs to support multiple opening-stock rows per warehouse. Do not silently assume
either way — ask if not already answered elsewhere in this system's build history.
2.3c COSTING METHOD CHANGE ON A USED ITEM: switching `costing_method` (weighted_avg ↔
FIFO) for an item that already has stock movements/COGS history is a significant
accounting-integrity concern, not a routine field edit — it changes how future COGS is
computed without recalculating past entries. Show the same warning as §2.2b, and consider
requiring elevated permission (Accountant+, matching §7) specifically for this field
change on an item with existing stock history, distinct from ordinary item edits.
...
3.1 TOGGLING "IS RETURNABLE" OFF: the same block condition used for deactivating an item
with unredeemed returnable credit (§3 above) applies equally to switching Is Returnable
off while leaving the item Active — if unredeemed Returnables Receipt credit exists
against this item, block the toggle with the same warning, rather than only checking this
at full deactivation. An item that still sells at checkout but silently stops being
treated as returnable would strand any outstanding BRR credit tied to it.
3.2 REGENERATING SKU/BARCODE ON A USED ITEM: regenerating either code on an item that
already has transaction history (sales, receipts, open POs referencing the current code)
requires an explicit confirmation warning — "This item has existing history under its
current code; regenerating may make old receipts/records harder to cross-reference." —
before proceeding. This is a warning, not a hard block, but must not happen silently the
way it can on a brand-new, never-used item.
...
4.1b ADD-PAGE COLLISION HANDLING: on the ADD page specifically, if a scanned/entered
barcode matches an existing item, do not let the user continue filling out and attempt
to save a whole new item only to fail on the unique-constraint at save time. Show the
"Open existing item?" prompt immediately on decode/entry, the same as on Edit, offering
to navigate straight to that item's Edit page instead of proceeding with a duplicate.
...
visible (server mirrors client). Linked-pair deposit values must match (§1.1) — reject a
save that would leave them inconsistent.

==================== 7 · PERMISSIONS / SECURITY ====================
Create/edit items: Inventory Manager+; view: all stock roles; manage returnable GL links:
Accountant+; audit all saves. Changing costing_method on an item with existing stock
history (§2.3c): Accountant+ specifically, even if the editing user is otherwise an
Inventory Manager with general edit rights.
...
10.4 Scan: decode fills barcode; existing-item prompt; generators unique; ADD-page
barcode collision offers to open the existing item instead of allowing a duplicate save
attempt (§4.1b).
10.5 GL: sale/purchase use item accounts; intake/redemption post §5.2; tax handling per
flag; chart-of-accounts confirmation that dropdown options match the accounts already
established in the POS Centre/Returnables/POS Suite builds (§0.3).
10.6 Screens match APPENDIX A (add + edit); no console errors; handlers untouched.
10.7 Linked-pair deposit consistency: editing one side of a linked filled/empty pair's
deposit value updates or validates against the other side per §1.1; a save that would
desync them is rejected.
10.8 Toggling Is Returnable off with unredeemed BRR credit against the item is blocked
with the same warning as deactivation (§3.1).
10.9 Regenerating SKU/barcode on a used item shows the history warning (§3.2); on a
brand-new never-used item it proceeds without it.
10.10 GL/costing-method field edits on a used item show the non-blocking warning (§2.2b/
§2.3c); a costing-method change specifically is restricted to Accountant+ (§7).
...
chart-of-accounts confirmation (§0.3); linked-pair deposit-consistency confirmation
(§1.1/10.7); used-item edit-warning confirmation (§2.2b/§2.3c/§3.1/§3.2); ADD-page
barcode-collision confirmation (§4.1b); opening-stock scope confirmation (§2.3b);
